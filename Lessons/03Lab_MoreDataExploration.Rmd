...
dataframe1 <- data.frame(d,e,f) #combining vectors into data frame
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) #rename columns
...
> Answer: types show up, matrics and data frame show upas data, vectors and symbol variable show up under values. when you define function, will show up as third category.
...
```{r DataExercise-1a}
highfarenheit <- c(81, 83, 83, 84, 78, 71, 79, 82, 85, 84)
highfarenheit

lowfarenheit <- c(55, 56, 58, 60, 61, 63, 63, 64, 64, 63)
lowfarenheit
```
...
```{r DataExercise-1b}
celsiusconversionhigh <- c((highfarenheit-32)*(5/9))
celsiusconversionhigh

celsiusconversionlow <- c((lowfarenheit-32)*(5/9))
celsiusconversionlow
```

Combine your four vectors into a data frame and add informative column names.

```{r DataExercise-1c}
newhigh <- as.data.frame(highfarenheit)
newhigh

temperatures <- cbind(newhigh, "new" = lowfarenheit, celsiusconversionhigh, celsiusconversionlow)
temperatures
...
fix.early.dates <- function(d, cutoff) { #cutoff is usually year we are at
