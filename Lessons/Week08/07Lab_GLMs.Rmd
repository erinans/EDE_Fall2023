...
```{r}
#Exercise 1:
OzoneYearDensity <- ggplot(EPAair, aes(x=Ozone, fill=as.factor(Year)))+
  geom_density(alpha=0.5)+
  scale_fill_manual(values=c("darkgreen", "darkblue"))+
  labs(x="Ozone AQI Value", y="Density", fill="")

print(OzoneYearDensity)

                       
OzoneYearBox <- ggplot(EPAair, aes(x=as.factor(Year), y=Ozone))+
  geom_boxplot()+
  stat_summary(geom = "text", 
               ...
               size = 4,
               label = c("b", "a"))+
  labs(y="Ozone AQI Vaue", x="Year")+
  ylim(0,150)
print(OzoneYearBox)


OzoneYearViolin <- ggplot(EPAair, aes(x=as.factor(Year), y=Ozone))+
...
#Exercise 2: Run a linear regression PM2.5 by Ozone(X). Find the p-value and R-squared value. 

OzoneLM <- lm(data = EPAair, PM2.5~Ozone)
summary(OzoneLM)
...
OzoneScatter <- ggplot(EPAair, aes(x=Ozone, y=PM2.5))+
  geom_point()+
