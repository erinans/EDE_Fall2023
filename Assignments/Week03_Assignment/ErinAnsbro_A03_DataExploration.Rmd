...
library(lubridate)
Neonics <-read.csv("./Assignments/Week03_Assignment/ECOTOX_Neonicotinoids_Insects_raw.csv", stringsAsFactors = TRUE) #importing data sets and renaming them
...
sort(summary(Neonics$Species.Common.Name)) #finding more info about Neonics, specifically species common name. Also using sort to find the most common.

```

> Answer: (Other) 670, Honey Bee 667, Parasitic Wasp 285, Buff tailed Bumblee 183, Carniolan Honey Bee 152, and Bumble Bee 140. These species are mostly bees, and bees pollinate. This would make sense with insecticide because pollinators go around to plants in order to pollinate.If the insecticide affects pollinators, that could have large ramifications for fertilization and reproduction of plants.
...
sort(summary(Neonics$Publication.Year)) #lowest date is 1982, highest is 2019, which means there are 38 bins
library(ggplot2)
ggplot(Neonics) +
  geom_freqpoly(aes(x=Publication.Year), bins = 38) #creating plot for studies per publication year #this is not right, need to change to date?
...
> Answer: Lab is most common, then fiedl natural, field artificial, field indeterminate.Ovettime, the amount of test locations in the lab increased. There was a peak of field natural in the latter half of the 2000s (around 2005-2010).
...
> Answer: NOEL and LOEL. Noel defined as No observable effect level which means the highest dose that produced effects not significantly different from the results of the control. LOEL is lowest observable effect level which means the lowest dose that produced effects that were significantly different from control results.
...
unique(Litter$collectDate) #finding unique collect dates, they are 2018-08-02 and 2018-08-30
```
...
> Answer: There is not a lot of results, making the violin plot appear thinner. Boxplot shows more summary, like interquartile range and median. This can be created from small and large data sets, so is a better visual for a data set that is not too dense.
