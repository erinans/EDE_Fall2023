...
```{r project.setup, eval=TRUE, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
#Import libraries
...
```{r, fig.height=11, fig.width=20, fig.align='left', echo=FALSE, warning=FALSE, fig.cap="Ozone Levels for 2018"}
#creating boxplot for Ozone levels in 2018
epa_data$Month<- factor(epa_data$Month)
Ozone2018 <- epa_data%>%
  filter(Year == 2018)%>%
  ggplot()+
  geom_boxplot(aes(x=Month, y=Ozone))+
  theme(axis.title = element_text(face="bold", size=18),axis.text =  element_text(face="bold", size=16))
print(Ozone2018)


```

```{r, fig.height=11,fig.width=20, fig.align='left', echo=FALSE, warning=FALSE, fig.cap="Ozone Levels for 2019"}
#creating boxplot for Ozone levels in 2019
Ozone2019 <- epa_data%>%
  filter(Year == 2019)%>%
...
## Git Repository
[my git link](https://github.com/erinans/EDE_Fall2023)
